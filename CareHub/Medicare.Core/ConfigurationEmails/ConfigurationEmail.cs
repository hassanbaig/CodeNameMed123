...
namespace CareHub.Core.ConfigurationEmails
{
    public static class ConfigurationEmail
    {
        public static void SignupEmail(string userID, string Pass, string toEmail)
        {
            try
            {
                ...
                msg.From = new System.Net.Mail.MailAddress(System.Configuration.ConfigurationManager.AppSettings["FromEmail"].ToString(), "CareHub Team");
                
                msg.To.Add(toEmail);

                msg.Subject = "Welcome to CareHub";
                msg.Body = swEmail.GetStringBuilder().ToString().Replace("[Customer]",userID).Replace("[Password]",Pass);
                msg.IsBodyHtml = true;

                CareHub.Core.Common.EmailHelper.SendMail(msg, Convert.ToInt32(System.Configuration.ConfigurationManager.AppSettings["EmailPort"].ToString()));
            }
            catch (Exception ex)
            {
                throw ex;
        ...
        public static void ForgotPasswordEmail(string userID, string Pass, string toEmail)
        {
            try
            {
                ...
                msg.Subject = "Your new password for CareHub";
                msg.Body = swEmail.GetStringBuilder().ToString().Replace("[Customer]", userID).Replace("[Password]", Pass);
                msg.IsBodyHtml = true;
