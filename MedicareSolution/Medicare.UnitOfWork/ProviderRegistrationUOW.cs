...
{ 
    public class ProviderRegistrationUOW : BaseUnitOfWork, IUnitOfWork
    {
        ...
        public ProviderRegistrationUOW(MedicareDevEntities context)
            : base(context)
        {
            if (context == null)
            {
        ...
        void IUnitOfWork.Save(AbstractDomainModel domainModel)
        {
            try
            {
                ...
                if (user == null)
                {
                    ...
                    user.Password = Medicare.Core.Common.Encryption.Encrypt("#", providerRegistration.Password);
                    user.UserRoleId = 1;
                    user.Enable = true;
                    user.Locked = false;
        ...
        public AbstractDomainModel GetAll(SearchCriteriaEnum searchCriteria)
        {
            ...
            {
                case Medicare.Core.Enumerations.SearchCriteriaEnum.GET_SPECIALTIES:
                    providerRegistration.Specialties = specialtyRepository.GetAll();
                    break;
                case Medicare.Core.Enumerations.SearchCriteriaEnum.GET_COUNTRIES:
                    providerRegistration.Countries = countryRepository.GetAll();
                    break;
                case Medicare.Core.Enumerations.SearchCriteriaEnum.GET_CITIES:
                    providerRegistration.Cities = cityRepository.GetAll();
                    break;
                case Medicare.Core.Enumerations.SearchCriteriaEnum.GET_LOCALITIES:
